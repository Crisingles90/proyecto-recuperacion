<?php
$dsn = 'mysql:dbname=ventas;host=127.0.0.1';
$usuario = 'root';
$clave = '';
...
function imprimetablaproducto($objeto,$form,$estilo="table",$editar=0){
	if ($objeto != NULL) {
		if (is_array($objeto)) {
			...
			foreach ($objeto as $datos) {
				...
				if ($editar != "0") {
					@$tabla .= "<td><a href=".$form."Buscar.php?id_producto=".$datos['id_producto'].">Modificar</a></dt>";
					@$tabla .= "<td><a href=".$form."Borrar.php?id_producto=".$datos['id_producto'].">Eliminar</a></td>";
				}
				$tabla .= "</tr>";
			}
		 $tabla .= "</table>";	
...
function imprimetablaventa($objeto,$form,$estilo="table",$editar=0){
	if ($objeto != NULL) {
		if (is_array($objeto)) {
			$tabla = "<table class=\"$estilo\">";
			$tabla .="<tr class'info'>";
